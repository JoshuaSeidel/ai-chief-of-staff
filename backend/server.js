...
if (!process.env.AICOS_AUTH_TOKEN && !process.env.API_TOKEN) {
  serverLogger.warn('AICOS_AUTH_TOKEN is not set; API token authentication is disabled');
}
...
app.use('/api/calendar', createRateLimiter({
  windowMs: 5 * 60 * 1000,
  max: Number(process.env.OAUTH_RATE_LIMIT_MAX || 40),
  keyPrefix: 'oauth',
  skip: req => !req.path.endsWith('/auth') && !req.path.endsWith('/callback')
}));
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const original = path.basename(file.originalname || 'upload.txt');
...
const upload = multer({ 
  ...
  fileFilter: (req, file, cb) => {
    ...
    const allowedMime = mimetype.startsWith('text/')
      || mimetype.startsWith('audio/')
      || mimetype === 'video/mp4'
      || mimetype === 'video/webm'
      || mimetype === 'application/octet-stream';

    if (!allowedUploadExtensions.has(extension) || !allowedMime) {
      return cb(new Error('Unsupported upload type. Upload text transcripts or supported audio/video recordings.'));
    }
